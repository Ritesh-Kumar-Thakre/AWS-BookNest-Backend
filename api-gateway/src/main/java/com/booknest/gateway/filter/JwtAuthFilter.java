...
public class JwtAuthFilter implements GlobalFilter, Ordered {
	...
	@org.springframework.beans.factory.annotation.Value("${internal.security.secret:BookNest_Internal_Secret_2024}")
	...
	public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
		...
		if (token != null) {
			...
			// No token
			if (isOpen) {
				ServerHttpRequest openRequest = request.mutate()
						.header("X-Internal-Secret", internalSecret)
						.build();
	...
	private boolean isOpenEndpoint(String path, String method) {
		...
		// Swagger and OpenAPI endpoints are open
		if (p.contains("/swagger-ui") || p.contains("/v3/api-docs")) return true;
