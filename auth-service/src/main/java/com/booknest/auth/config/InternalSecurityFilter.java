...
/**
 * Security filter that blocks direct access to this microservice.
 * Only requests forwarded by the API Gateway (which include the
 * X-Internal-Secret header) are allowed through.
 *
 * Swagger/Actuator endpoints are exempted so developers can still
...
public class InternalSecurityFilter implements Filter {
    ...
    public void doFilter(ServletRequest req, ServletResponse res,
        ...
        // Allow Swagger and Actuator endpoints without the secret
        // (so developers can still browse API docs locally)
